...
const SITE_TITLE = "SMP 4 SIGI | SMP Negeri 4 Sigi Dolo Kabupaten Sigi";
...
function setActivePage(pageId, pushHash = true) {
  ...
  // FIX: jangan lagi berubah jadi "Beranda | SMP 4 SIGI"
...
function updateHeaderState() {
  if (siteHeader) siteHeader.classList.toggle('is-scrolled', window.scrollY > 20);
  if (backTop) backTop.classList.toggle('show', window.scrollY > 360);
}
...
function tickClock() {
  ...
  if (liveClock) {
    liveClock.textContent = formatter.format(new Date()).replace(/\./g, ':') + ' WITA';
  }
}
...
if (modal && modalImage && modalVideo && modalVideoSource) {
  document.querySelectorAll('.gallery-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (type === 'video') {
        ...
        modalImage.src = fullFile || image.src;
      }
